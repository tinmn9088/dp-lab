...
import by.nikita.data.cards.table.actions.PutCardsToResultDeckAction;
...
public class PutCardsToResultDeckActionProcessor extends UserInputProcessor {

    public PutCardsToResultDeckActionProcessor(CarltonSolitaireCardTable carltonSolitaireCardTable) {
    ...
    public void process(String userInput) {
        ...
        if (userInput.matches("put\\s+to\\s+\\d+\\s+result\\s+deck")) {
            String[] parts = userInput.split("\\s+");
            int deckNumber = parseInt(parts[2]) - 1;
            List<Card> takenCards = getTakenCards();

            Action action = new PutCardsToResultDeckAction(getCarltonSolitaireCardTable(), deckNumber, takenCards);
